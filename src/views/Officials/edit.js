...
const Create = (props) => {
  ...
  const getData = async () => {
    if (props._id && props._id != 0)
      await requestPost('Crud/fetch-single', {
        collection: 'users',
        where: { user_id: props._id }
      }).then((res) => {
        if (res.data) {
          setDefaultImage(process.env.REACT_APP_PUBLIC_URL + res.data.avatar);
          setUserData(res.data);

          ///calculate states
          const country_id = countries.filter((country) => country.name === res.data.country)[0].id;
          setStatesList(states.filter((state) => state.country_id === country_id));
          ///calculate cities
          const state_id = states.filter((state) => state.name === res.data.state)[0].id;
          getCities(state_id);
        }
      });
  };
