...
const Edit = (props) => {
  ...
  const getData = async () => {
    ...
    if (props._id && props._id != 0)
      await getOne(_id).then(async (res) => {
        ...
        setValid_upto(res.userData.valid_upto);
        delete res.userData.valid_upto;
        setUserData(res.userData);
        setBankData({ official_id: res.userData.official_id, ...res.bankData });
      });
  ...
  const handleSubmit = async (event) => {
    ...
    setLoading(true);
    requestPost('officials/update', userData).then(async (res) => {
      if (res.code != 0) toast.warning(res.message);
      else
        await requestPost('officials/update_banks', bankData).then(async () => {
          onSuccess();
